...
#include "pico/platform.h"   // __not_in_flash_func: keep the scan ISR in RAM so a second core's flash
                             // (XIP) traffic can't delay it -> no scan-timing jitter under load.
#endif
...
#if defined(ARDUINO_ARCH_RP2040)
void __not_in_flash_func(scan_running_dmd_R)()
#else
