...
// --------------- //
// dmd_out_spwm12  //   (HUB320 / 4 data groups = 12 parallel data lines)
// --------------- //
// Identical to dmd_out_spwm but shifts 12 data bits per CLK (out pins,12 + out null,4) and uses a
// 16-bit autopull threshold, so the DMA feeds one uint16 per column-bit instead of one byte.

#define dmd_out_spwm12_wrap_target 0
#define dmd_out_spwm12_wrap 11

static const uint16_t dmd_out_spwm12_program_instructions[] = {
    //     .wrap_target
    0x80a0, //  0: pull   block           side 0
    0x6030, //  1: out    x, 16           side 0
    0x6050, //  2: out    y, 16           side 0
...
#if !PICO_NO_HARDWARE
static const struct pio_program dmd_out_spwm12_program = {
    .instructions = dmd_out_spwm12_program_instructions,
    .length = 12,
    .origin = -1,
};

static inline pio_sm_config dmd_out_spwm12_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + dmd_out_spwm12_wrap_target, offset + dmd_out_spwm12_wrap);
    sm_config_set_sideset(&c, 1, false, false);
    return c;
}

static inline pio_sm_config dmd_out_spwm12_program_init(PIO pio, uint sm, uint offset,
    int clk_div,
    int out_pins_base, int out_pins_cnt,
    int set_pin_base,
    int side_pin_base)
{
    pio_sm_config c = dmd_out_spwm12_program_get_default_config(offset);
    sm_config_set_clkdiv(&c, clk_div);
    ...
    // rightshift = true, autopull = true, threshold = 16 (one uint16 per column-bit)
